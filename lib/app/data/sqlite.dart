...
class DatabaseHelper {
  ...
  Future<void> insertProduct(Cart productModel) async {
    ...
    if (existingProducts.isNotEmpty) {
      // Product already in the cart, update the count
      Cart existingProduct = Cart.fromMap(existingProducts.first);
      existingProduct.count += productModel.count;
      await db.update(
        'Cart',
        existingProduct.toMap(),
        where: 'productID = ?',
        whereArgs: [existingProduct.productID],
      );
    } else {
      // Product not in the cart, insert new product
      await db.insert(
        'Cart',
        productModel.toMap(),
